...
def load_data(file_path):
    EEG51 = os.path.join(settings.get("eeg_folder"), "_dot_MusicTherapy_51EEGECG_1Sync.txt")
    EEG62 = os.path.join(settings.get("eeg_folder"), "_dot_MusicTherapy_62EEG_1ECG_1Sync.txt")
    # 创建信息对象
    ...
    ch_names = np.genfromtxt(EEG51, dtype=str, delimiter=',',
                             usecols=[3]).tolist()
    analyseThread = ObjectManager.get("analyseThread")
    analyseThread.sendLog.emit("Fitting ICA to data using 51 channels (please be patient, this may take a while)")
    ch_exclude = []
    # 通过判断电极数量，选择使用哪种电极配置文件
    # if n_channels == 62:
    #     ch_62_name = np.genfromtxt(EEG62, dtype=str, delimiter=',',
    #                                usecols=[3]).tolist()
